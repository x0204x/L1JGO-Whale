...
## 地圖資料納入版本控制 — server/map/ 1031 個 tile CSV（總計 158MB）

- 移除 `server/.gitignore` 內 `map/` 排除規則（原註解「二進制太大不上傳」屬舊有保守設定，實測為純 CSV ASCII，最大單檔 7.5MB 遠低於 GitHub 50MB 警戒線）。
- 補入完整 `server/map/` 目錄 1031 個 `.txt` 圖磚資料，方便他人 clone 後直接啟動伺服器、不需另外取得地圖檔。
